...
for dataset_dir in tqdm(DATASET_DIRS, desc="Dataset Directories"):
    if not os.path.isdir(dataset_dir):
        ...
        continue
    label_name = os.path.basename(dataset_dir)
...
def map_fn_low(img_path, label):
    try:
        ...
        return tf.zeros((IMG_SIZE_LOW, IMG_SIZE_LOW, 1), dtype=tf.float32), label
...
def map_fn_high(img_path, label):
    try:
        ...
        # Handle corrupted images by returning a black image
        return tf.zeros((IMG_SIZE_HIGH, IMG_SIZE_HIGH, 1), dtype=tf.float32), label


# 6. Create TensorFlow Datasets

# Low-resolution dataset
dataset_low = tf.data.Dataset.from_tensor_slices((img_paths, labels))
dataset_low = dataset_low.map(map_fn_low, num_parallel_calls=tf.data.AUTOTUNE)

# High-resolution dataset
dataset_high = tf.data.Dataset.from_tensor_slices((img_paths, labels))
...
try:
    img_sample_low, label_low = next(iter(dataset_low.batch(1)))
    img_sample_high, label_high = next(iter(dataset_high.batch(1)))
...
def create_super_resolution_dataset(dataset_high, batch_size):
    def map_fn(x, y):
        low_res = tf.image.resize(x, (IMG_SIZE_LOW, IMG_SIZE_LOW))
        return (low_res, x)  # Return (low_res, high_res)
    dataset_sr = dataset_high.map(map_fn)
...
for epoch in tqdm(range(EPOCHS), desc="Training Epochs"):
    epoch_disc_loss = []
    epoch_gen_loss = []
    for images, _ in tqdm(dataset_high.batch(BATCH_SIZE), desc=f"Epoch {epoch+1}", leave=False):
        try:
...
for _ in tqdm(range(number_of_images_to_generate // batch_size_gen), desc="Generating Images"):
    ...
    try:
        generated_image = generator(noise, training=False)  # Generate 128x128 images
...
# 23. Prepare Data for CNN Training

# Use the real dataset for CNN training
dataset_cnn = dataset_high.batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
...
dataset_cnn = dataset_cnn.shuffle(buffer_size=BUFFER_SIZE)

train_dataset = dataset_cnn.take(train_size // BATCH_SIZE)
val_dataset = dataset_cnn.skip(train_size // BATCH_SIZE)
...
# Prepare test dataset (using validation dataset for simplicity)
